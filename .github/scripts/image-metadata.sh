#!/usr/bin/env bash
# Print GitHub step outputs. Normalize image repositories, NOT case-sensitive tags.
set -euo pipefail
export LC_ALL=C
...
if [[ ! "$repository" =~ ^${component}/${component}$ ]]; then
  echo 'Invalid owner/repository for a container image' >&2
  exit 1
fi
if [[ ! "$registry" =~ ^[a-z0-9][a-z0-9.-]*(:[0-9]+)?$ ]]; then
  echo 'Invalid container registry' >&2
  exit 1
fi
if [[ ! "$GITHUB_SHA" =~ ^[0-9a-f]{40}$ ]]; then
  echo 'Expected a full GitHub commit SHA' >&2
  exit 1
fi
...
if [[ ! "$release_tag" =~ ^[A-Za-z0-9_][A-Za-z0-9_.-]{0,127}$ ]]; then
  echo 'Invalid container release tag' >&2
  exit 1
fi
