...
const require = createRequire(
	process.env.PLAYWRIGHT_RUNTIME
		? path.join(process.env.PLAYWRIGHT_RUNTIME, "package.json")
		: import.meta.url,
);
...
let groups = [
	{
		...
		enabled: true,
		cron_expr: "",
		export_sort: "speed_desc",
		export_include_dead: false,
		created_at: "2026-09-01T00:00:00Z",
	},
	{
		id: "group-asia",
		name: "Asia direct node",
		kind: "node",
		url: "ss://fixture-private",
		enabled: false,
		cron_expr: "",
		export_sort: "speed_desc",
		export_include_dead: false,
...
await context.route("**/api/**", async (route) => {
	...
	if (
		...
		return route.fulfill({
			...
			body: JSON.stringify({
				code: "unavailable",
				message: "Intentional UI fixture failure",
			}),
		});
	const data = {
		...user,
		...
		nodes: [],
		rules: [],
		jobs: lower.includes("latest") ? latestJobs : [],
		channels: [],
		tags: [],
		results: [],
	};
...
try {
	...
	await page
		.getByRole("heading", { name: "Your network, at a glance" })
		.waitFor();
	await page
		.getByRole("button", { name: /Europe network/ })
		.first()
		.waitFor();
	assert.equal(
	...
	await page
		.locator('[data-slot="workspace-sidebar"] button[aria-controls]')
		.click();
	assert.equal(
		await page
	...
	await page
		.getByRole("button", { name: "Add node group", exact: true })
		.first()
		.click();
	...
	await page
		.getByRole("heading", { name: "Node groups", exact: true })
		.waitFor();
	await page
		.getByRole("searchbox", { name: "Search node groups" })
		.fill("Europe");
	assert.equal(
		await page.getByRole("button", { name: /Asia direct node/ }).count(),
		0,
	);
	...
	await page
		.getByRole("dialog")
		.getByRole("button", { name: "Scheduler", exact: true })
		.click();
	await page.getByRole("heading", { name: "Scheduler", exact: true }).waitFor();
	await page.getByRole("dialog").waitFor({ state: "hidden" });
	...
	await page.goto(origin);
	await page
		.getByRole("heading", { name: "Unable to load node groups", exact: true })
	...
	await page.reload();
	await page
		.getByRole("heading", { name: "Bring your nodes together.", exact: true })
		.waitFor();
	...
	await writeFile(
		...
		JSON.stringify(
			{
				backend:
					"mocked fixtures only; no real account or backend was contacted",
				checks,
				requests: [...requests],
				errors,
			},
	...
	console.log(
		JSON.stringify(
			{ passed: checks.length, checks, requests: [...requests] },
			null,
			2,
		),
	);
	...
	await writeFile(
		...
		JSON.stringify(
			{ message: String(error), requests: [...requests], errors },
			null,
			2,
		),
	);
