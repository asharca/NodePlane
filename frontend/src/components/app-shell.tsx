...
const navigationGroups = [
	{
		id: "workspace",
		title: "WORKSPACE",
		items: WORKSPACE_NAV.map(({ id, label, icon: Icon }) => ({
			id,
			label,
			icon: <Icon />,
		})),
...
export function AppShell({ children }: { children: ReactNode }) {
	...
	useEffect(() => {
		try {
			...
			setCollapsed(
				saved === null ? window.innerWidth < 1180 : saved === "true",
			);
		} catch {
			...
			<WorkspaceShell
				...
				sidebar={
					<WorkspaceSidebar
						...
						onSelect={(id) => {
							const target = WORKSPACE_NAV.find((item) => item.id === id);
							if (target) void navigate({ to: target.to });
						}}
						footer={
							<div className="rounded-xl border border-border/60 bg-background/60 p-3">
								<p className="text-xs font-medium">Your network, in focus.</p>
								<p className="mt-1 text-xs leading-5 text-muted-foreground">
									Subscriptions, checks and automation in one workspace.
								</p>
							</div>
						}
					/>
				...
				header={
					<header className="flex h-14 min-w-0 items-center justify-between gap-3 border-b border-border/70 px-4 sm:px-5">
						<nav
						...
						<div className="flex shrink-0 items-center gap-1.5">
							...
							<DropdownMenu>
								<DropdownMenuTrigger
									...
									className="grid size-9 place-items-center rounded-xl border border-border bg-muted/60 text-xs font-semibold outline-none hover:bg-muted focus-visible:ring-2 focus-visible:ring-ring"
								>
									{me ? (
