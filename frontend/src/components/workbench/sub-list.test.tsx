...
describe("node group navigation", () => {
	it("searches groups and resets empty results", () => {
		render(list());
		fireEvent.change(
			screen.getByRole("searchbox", { name: "Search node groups" }),
	...
	it("filters paused groups and selects by stable id", () => {
		const select = vi.fn();
		render(list({ onSelect: select }));
	...
	it("does not leak subscription tokens in visible text", () => {
		const { container } = render(list());
		expect(container.textContent).not.toContain("secret");
	});
	it("keeps loading distinct from an empty account", () => {
		render(list({ subs: [], loading: true }));
		expect(screen.queryByText("Your first node group")).toBeNull();
