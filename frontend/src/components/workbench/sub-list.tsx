...
export function SubList({
	...
	return (
		<section
			aria-label="Node groups"
			className="flex h-full min-h-0 flex-col bg-card/60"
		>
			<div className="shrink-0 space-y-4 p-4">
				<div className="flex items-center justify-between gap-2">
					<div className="flex items-center gap-2">
						<h1 className="text-sm font-semibold">Node groups</h1>
						<Badge>{loading ? "…" : subs.length}</Badge>
					</div>
					<Button
						size="icon-sm"
						aria-label="Add node group"
				...
				>
					{filters.map(({ id, label }) => (
						<button
							...
							className={cn(
								"min-h-8 min-w-0 flex-1 rounded-lg px-2 text-xs font-medium transition-colors",
								filter === id
			...
			>
				{loading ? (
					[0, 1, 2, 3].map((key) => (
						<div
							key={key}
							className="space-y-3 rounded-xl border border-border/50 p-3.5"
						>
					...
					<EmptyState
						icon={subs.length ? SearchX : Inbox}
						title={subs.length ? "No matching groups" : "Your first node group"}
						description={
							subs.length
								? "Try a different name or reset the filters."
								: "Add a subscription or a direct node to get started."
						}
						className="m-0 border-0 px-2 py-8"
						action={
							subs.length ? (
								<Button
									variant="outline"
					...
					visible.map((sub) => {
						...
						const percent =
							selected && liveProgressPct !== null
								? Math.max(0, Math.min(100, Math.round(liveProgressPct)))
								: null;
						return (
							<button
								key={sub.id}
								...
								className={cn(
									"group w-full rounded-xl border p-3.5 text-left outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-card",
									selected
										? "border-primary/25 bg-background shadow-xs"
							...
							>
								...
								<p className="mt-1.5 truncate pl-4 text-xs text-muted-foreground">
								...
								{running && percent !== null ? (
									<Progress
										value={percent}
										label={`Checking ${groupDisplayName(sub)}`}
										className="mt-3 h-1"
									/>
								) : latest?.finished_at ? (
									<p className="mt-2 pl-4 text-[10px] text-muted-foreground">
										Checked {relativeTime(latest.finished_at)}
									</p>
								) : null}
							</button>
						);
			...
			<footer className="flex shrink-0 items-center justify-between border-t border-border/60 px-4 py-3 text-[11px] text-muted-foreground">
				<span>
					{loading
						? "Loading groups…"
						: `${visible.length} of ${subs.length} groups`}
				</span>
				<span>NodePlane</span>
			</footer>
		</section>
