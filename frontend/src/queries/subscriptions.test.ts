...
describe("create node group transaction", () => {
	it("creates a subscription without attempting node import", async () => {
		await createNodeGroup({ params: { kind: "subscription", url: "https://example.invalid/sub" } });
		expect(api.importNodes).not.toHaveBeenCalled();
		expect(api.remove).not.toHaveBeenCalled();
	});
	it("rejects an empty single-node input before creating a group", async () => {
		await expect(createNodeGroup({ params: { kind: "node" }, nodeContent: " " })).rejects.toThrow("Paste one node share link");
		expect(api.create).not.toHaveBeenCalled();
	});
	it("imports the node into the group created by the same operation", async () => {
		const result = await createNodeGroup({ params: { kind: "node" }, nodeContent: " fixture-link " });
	...
	it("removes the new group and preserves the original import error", async () => {
		...
		await expect(createNodeGroup({ params: { kind: "node" }, nodeContent: "invalid" })).rejects.toBe(error);
	...
	it("reports a failed cleanup with the affected group instead of claiming rollback", async () => {
		...
		await expect(createNodeGroup({ params: { kind: "node" }, nodeContent: "invalid" })).rejects.toThrow("Remove node group new-fixture-group before retrying");
	});
	it("never deletes anything when initial creation fails", async () => {
		api.create.mockRejectedValue(new Error("offline"));
		await expect(createNodeGroup({ params: { kind: "node" }, nodeContent: "node" })).rejects.toThrow("offline");
		expect(api.remove).not.toHaveBeenCalled();
		expect(api.importNodes).not.toHaveBeenCalled();
	});
});
