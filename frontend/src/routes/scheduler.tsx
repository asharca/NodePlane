...
function SchedulerPage() {
	...
	const subName = (id: string) => {
		...
		return sub ? groupDisplayName(sub) : id.slice(0, 8);
	};
	const toggleMut = useSetScheduleEnabled();
	const deleteMut = useDeleteScheduledJob();
	const visible = jobs.filter((job) =>
		`${subName(job.subscription_id)} ${job.cron_expr} ${describeCron(job.cron_expr)}`
			.toLowerCase()
	...
	return (
		<div className="h-full overflow-y-auto">
			<Page as="div" className="max-w-6xl pb-10">
				<PageHeader
					title="Scheduler"
					description="Set your checking routine. Keep recurring work out of your way."
					actions={
						<Button
							onClick={openNew}
							disabled={subsQuery.isLoading || subsQuery.isError}
						>
				...
				{subsQuery.isError && (
					<RequestError
						title="Node groups could not be loaded"
						description="Retry before creating or editing a schedule."
				...
				{jobsQuery.isError ? (
					...
					<>
						<div className="grid grid-cols-3 gap-3 sm:gap-4">
							{[
								{ label: "Total schedules", value: jobs.length },
								{
									label: "Enabled",
						...
						<Card padded={false} className="overflow-hidden">
							<div className="flex flex-wrap items-center justify-between gap-3 border-b border-border p-4 sm:px-5">
								<div>
									<h2 className="text-sm font-semibold">Scheduled checks</h2>
									<p className="mt-1 text-xs text-muted-foreground">
										Next-run times are estimates in your browser's timezone.
									</p>
								</div>
								<SearchInput
							...
							{jobs.length === 0 ? (
								<EmptyState
									icon={CalendarClock}
									title="Put your checks on a schedule"
									description="Create a recurring check for a node group. You can pause it at any time."
									action={
										<Button
											onClick={openNew}
											disabled={subsQuery.isLoading || subsQuery.isError}
								...
								<EmptyState
									...
									description="Try a group name or a cron expression."
									action={
								...
								<div className="overflow-x-auto">
									<table className="surface-table min-w-[720px]">
										<caption className="sr-only">
											Recurring checks, next-run estimates, and schedule
											controls
										</caption>
										<thead>
											<tr>
												...
												<th scope="col">Latest group check</th>
													...
													<tr key={job.id}>
														<td>
															<button
																...
																className="max-w-52 truncate text-left font-medium hover:text-primary hover:underline"
																onClick={() => {
																	setEditing(job);
																	setDialogOpen(true);
																}}
														...
														<td>
															...
															<code className="mt-1 inline-block rounded-md bg-muted px-1.5 py-0.5 text-[11px] text-muted-foreground">
																{job.cron_expr}
															</code>
														</td>
														...
														<td>
															<Switch
																aria-label={`Enable schedule for ${name}`}
																checked={job.enabled}
