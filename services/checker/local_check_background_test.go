...
func TestBackgroundUnlockUsesExplicitOwner(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if len(result.Platforms) != 1 || !result.Platforms["owned_local"].Unlocked { t.Fatalf("wrong owner's rules: %#v", result.Platforms) }
	if _, found := result.Platforms["foreign_local"]; found { t.Fatal("another user's rule leaked into the report") }
}

func TestBackgroundUnlockRequiresOwner(t *testing.T) {
	if _, err := GetLocalUnlockForUser(context.Background(), ""); err == nil { t.Fatal("missing owner must be rejected") }
}
